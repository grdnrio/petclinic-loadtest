...
do_exec() {
  ...
  locust --host=http://$TARGET_HOST -f $LOCUST_FILE --clients=$CLIENTS --hatch-rate=$HATCH_RATE --run-time 3m --no-web --only-summary
  echo "done"
}
...
Options:
  ...
  -r  Hatch rate - number of user per second (default 5)
Description:
